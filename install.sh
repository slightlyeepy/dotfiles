...
if [ "$answer" != "${answer#[Yy]}" ]; then
	...
	if [ "$answer" != "${answer#[Yy]}" ]; then
		...
		if [ ! -d ~/.local/share/icons/catppuccin-mocha-mauve-cursors ]; then
			...
			curl -fLO https://github.com/catppuccin/cursors/releases/download/v0.3.1/catppuccin-mocha-mauve-cursors.zip
			unzip catppuccin-mocha-mauve-cursors.zip
			cp -r catppuccin-mocha-mauve-cursors ~/.local/share/icons/
			rm -r catppuccin-mocha-mauve-cursors.zip catppuccin-mocha-mauve-cursors
		fi
		sed -i '/gtk-cursor-theme-name=".*"/d' ~/.gtkrc-2.0 2>/dev/null
		echo 'gtk-cursor-theme-name="catppuccin-mocha-mauve-cursors"' >> ~/.gtkrc-2.0
		if [ -f ~/.config/gtk-3.0/settings.ini ]; then
			...
			sed -i '/[Settings]/a gtk-cursor-theme-name=catppuccin-mocha-mauve-cursors' ~/.config/gtk-3.0/settings.ini
		else
			maybe_mkdir ~/.config/gtk-3.0
			echo '[Settings]' > ~/.config/gtk-3.0/settings.ini
			echo 'gtk-cursor-theme-name=catppuccin-mocha-mauve-cursors' >> ~/.config/gtk-3.0/settings.ini
		fi
		...
		echo 'Inherits=catppuccin-mocha-mauve-cursors' > ~/.local/share/icons/default/index.theme
